...
- Depósito en buzón interno como canal de notificación (ver sección 5, antes RF-08.3).
...
| Departamento | Equipo/área de negocio destinataria de la notificación (ticket o email, según RF-08); consulta lo que le llega. |
...
| **Sub-requisitos** | RF-01.1 Invocar `localiza()` de forma periódica (frecuencia configurable) para obtener el listado de envíos pendientes en cualquiera de los PUC integrados. Si la lista devuelta está vacía, el sistema no ejecuta ningún procesamiento adicional y simplemente espera hasta el siguiente ciclo de sondeo.<br>RF-01.2 Si la lista no está vacía, iterar sobre cada elemento. El momento de `peticionAcceso()` depende de `tipoEnvio`, capturado en `localiza()` (no vuelve en `peticionAcceso()`). (a) **`1` comunicación** — invocar `peticionAcceso()` en el mismo ciclo. Acceder no tiene efectos jurídicos ni plazo de lectura, y DEHú no genera acuse (FAQ DEHú). (b) **`2` notificación** — `peticionAcceso()` es la comparecencia: practica la notificación y puede abrir el plazo de alegaciones o recursos. El plazo de comparecencia va de la fecha de puesta a disposición a la de caducidad, ambas del organismo emisor; DEHú no las gestiona (las gestiona el PUC). Cuándo invocar `peticionAcceso()` respecto de ese plazo queda pendiente de las áreas de negocio.<br>RF-01.3 Deduplicar mediante el identificador único devuelto por DEHú, para garantizar idempotencia ante reintentos o fallos parciales.<br>RF-01.4 Registrar y reintentar (con backoff) los fallos de conectividad, expiración de certificado o errores SOAP, sin perder el envío de la cola de pendientes. |
...
| **Criterio de aceptación** | El 100% de las notificaciones comparecidas en el día tienen su acuse y sus anexos-referencia almacenados antes de que expire la ventana de 24h. Las comunicaciones no tienen acuse; sí tienen documento principal y anexos-referencia. |
...
| **Sub-requisitos** | RF-08.1 Crear el ticket en la cola correspondiente cuando el sistema de ticketing es el canal configurado para ese departamento/tipo de comunicación (comportamiento por defecto, **único canal obligatorio del MVP**).<br>RF-08.2 *(deseable, no obligatorio en el MVP — ver nota de diseño)* Enviar un email al departamento/equipo destino como canal alternativo, configurable por departamento con independencia de si el ticket es alcanzable, o activable como contingencia cuando el sistema de ticketing no está disponible o resulta inviable — ambos casos son válidos, no exclusivamente el segundo.<br>RF-08.4 La selección de canal debe resolverse por configuración/reglas, no por código específico de cada caso, para que añadir un canal nuevo no implique modificar RF-01 a RF-07.<br>RF-08.5 Confirmar la ejecución (idempotencia: no duplicar la acción si ya se ejecutó para el mismo identificador DEHú) y registrar el resultado (éxito/fallo), con reintento en caso de fallo transitorio del canal de destino. **Excepción confirmada**: en una reclasificación posterior al envío inicial, el ticketing no soporta transferir de cola (ver sección 5.2), por lo que el flujo correcto es finalizar el ticket original y crear uno nuevo en la cola correcta — este segundo ticket no debe tratarse como duplicado, sino como resultado esperado de RF-09/reclasificación, ya sea manual (Operador) o automática (RF-10, Agente IA vía MCP). |
| **Nota de numeración** | RF-08.3 (depósito en buzón interno) queda retirado del alcance activo del MVP — ver sección 5. Se mantiene el hueco en la numeración (RF-08.1, RF-08.2, RF-08.4, RF-08.5) en lugar de renumerar, porque RF-08.4 y RF-08.5 ya están referenciados por número desde RF-09.6, RF-10.4, RF-11.5, `TODO.md` y `Diagrama_Clases.md`. Si el buzón se retoma más adelante, se reincorpora como RF-08.3 sin afectar al resto. |
| **Nota de diseño — prioridad del canal ticket** | El director de empresa confirmó que el interés real está en el canal ticket: el email (RF-08.2) es deseable pero no bloqueante para el cierre del MVP, con independencia de si el ticket es alcanzable o no en un caso concreto. No cambia la arquitectura — `RF-08.4` ya desacopla la selección de canal del resto del pipeline —, solo la prioridad de desarrollo y el criterio de aceptación. |
| **Criterio de aceptación** | Toda comunicación clasificada con éxito produce exactamente una acción de notificación vía el canal ticket, en un tiempo máximo definido por SLA (email, RF-08.2, no es requisito de cierre — ver nota de diseño). |
...
| **Nota de diseño** | Este requisito responde a una propuesta explícita de negocio (manager de empresa) de automatizar la corrección de errores de clasificación sin depender de que el Operador esté disponible, apoyándose en el mismo mecanismo de eventos ya establecido en RF-07/RF-10.1. |
...
| **Nota técnica** | La distinción editable/no editable no depende del campo `tipoEnvio` de DEHú. Ese campo sí distingue el envío: `1` comunicación, `2` notificación (plan de pruebas funcionales para Gran Destinatario v2.0, SGAD, 18/03/2024, §2.1.2 y §2.1.3). Lo editable depende de si la comunicación ya generó una acción de derivación (ticket u otro canal) en el propio sistema. Lo que DEHú entrega es siempre de solo lectura; lo editable es únicamente lo que el sistema propio ha producido a partir de ello. Una comunicación en cola de revisión pendiente (RF-09), al no tener todavía ninguna derivación generada, es de solo lectura bajo esta misma regla. |
...
| **Retención del texto extraído** | El texto extraído del documento principal (RF-03) se almacena en una tabla independiente de `INTERPRETACION`, para no penalizar el rendimiento de las consultas habituales sobre comunicaciones. Su periodo de retención se define mediante un **parámetro global configurable** (no codificado), de forma que pueda ajustarse sin desplegar cambios de código. El valor concreto se fijará tras contrastarlo con los compañeros. |
...
- **Depósito en buzón interno como canal de notificación** (antes RF-08.3) — mecanismo mencionado en la propuesta original de MGS como "ya existente", pero retirado del alcance activo del MVP por ahora. RF-08.4 (selección de canal por configuración, no por código específico) ya está pensado para no requerir cambios en el resto del pipeline cuando se retome — ver nota de numeración en RF-08.
...
|---|---|
| `tipoEnvio` no decide si un registro es editable | La leyenda ya está confirmada (`1` comunicación, `2` notificación; plan de pruebas GD v2.0, §2.1.2–2.1.3). RF-11 sigue sin usarlo: lo editable es si ya hay una derivación en el sistema propio. |
| Comparecencia según `tipoEnvio` | Comunicación (`1`): descarga en el mismo ciclo; sin plazo de lectura, sin efectos jurídicos y sin acuse (FAQ DEHú, 23/09/2026). Notificación (`2`): `peticionAcceso()` comparece y puede arrancar el plazo de respuesta; el momento sigue pendiente de las áreas. El plazo operativo lo fija el emisor (puesta a disposición → caducidad) y lo gestiona el PUC, no DEHú. Los diez días naturales son el rechazo tácito de la Ley 39/2015, art. 43.2. |
...
| El sistema de Ticketing solo permite cambiar el estado del ticket a nivel backend (sin campo de motivo en frontend) — confirmado con el equipo de Ticketing | No es posible registrar explícitamente el motivo de un cambio de estado (p. ej. una reclasificación) dentro del propio ticket; el origen del cambio debe inferirse a partir del evento que lo generó (RF-07/RF-10), no de un campo de texto libre. Afecta a la trazabilidad de RF-09/RF-10 si en el futuro se requiere justificación explícita legible por un humano. |
| La "audiencia back" (componente desplegable independiente dentro de Ticketing, pensado para exponer APIs a consumidores backend externos) **no se sabe si ya expone los endpoints necesarios** para que la aplicación pueda invocar la modificación de un ticket existente (RF-11.4/RF-11.5) — se están solicitando/ampliando por otro compañero, sin fecha ni detalles confirmados | Bloquea el cierre técnico de RF-11 (modificación de notificaciones); el diseño de alto nivel (edición in-place vs. finalizar+crear) puede avanzar, pero su implementación y validación quedan condicionadas a que se confirme qué endpoints existen o se publiquen. |