...
- **Diagrama de clases en capas** (`modelo`/`aplicacion`/`infraestructura`/`api`) — ya extendido a cuatro rebanadas verticales: RF-09 (aceptar/reclasificar, cerrada con revisión previa), RF-08 (ejecución de la acción resultante), RF-10 (reclasificación automática vía Agente IA) y RF-11.4 (modificación in-place) — ver `Diagrama_Clases.md`. RF-11.5 (cambio de departamento en RF-11) queda pendiente de diagramar, condicionada a la API "audiencia back" de Ticketing.
