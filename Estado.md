...
- Clasificación sin leer el documento (23/09/2026, escrita en RF-05 y en el PRD; pendiente de confirmar con el resto de departamentos): organismo emisor y concepto de `localiza()`, antes de abrir. Si la confianza no basta, revisión y no se abre. El resumen solo se hace en la descarga inmediata: la comunicación, en el mismo ciclo. La notificación no se resume mientras la descarga no sea inmediata (pendiente de confirmar con el resto de departamentos). El anexo no entra a clasificar.
- Avisos y reparto (25/09/2026; detalle en `_local/reuniones/` y `_local/envio`). Se trata lo que se lee en DEHú. El aviso de la sede de la DGSFP (`notificaciones.dgsfp@economia.gob.es`) no es fuente: trae área remitente y nº de registro, y no trae concepto. El de DEHú (`noreply.dehu@correo.gob.es`) trae concepto y, si es electrónica, vencimiento; no trae área remitente. Esos dos campos tampoco están en `localiza()` documentado. El enlace del aviso DEHú acaba en `/aceptar`. Dentro del DIR3 `E00119006`, Red de Mediación se guía por área remitente «Distribución» y el aviso DEHú de concepto `Modelo 2B.xml` va al SAC: el emisor no separa departamentos. Falta ver de dónde sale esa etiqueta y reproducir el reparto, sin preguntar al área y sin abrir documentos (`TODO.md`). Asesoría Fiscal reparte, a grandes rasgos: Inspección, análisis de balances, Solvencia y DEC → Coordinación DGS; Modelo 2B y Reclamaciones → SAC; Exclusivos y Mediación → Red de Mediación. Solo `Modelo 2B.xml` se ha visto escrito en el campo Concepto. Otras muestras, titular `A08171373`: AEAT `EA0028512`, concepto «Notificación administrativa»; TGSS `EA0042298`, embargo `NOT. DIL.LEV.EMB. PERS. O ENTID. PAGAD.` → Fiscal, el resto lo descarga RRHH; Catastro `E00127105`, aviso postal, `ACUERDO DE BAJA DE TITULARIDAD`, sin vencimiento, art. 42.1. Red de Mediación: 2–3 al mes, adjunto ZIP, plazo habitual 14 días; la descarga automática no les perjudica. Quieren delegar altas y bajas de mediadores sin certificado.
